...
class WpPasswordPolicyManager {
	...
	protected function GetPasswordRules() {
		$rules         = array(
			esc_html__( 'not be the same as your username', 'wp-password-policy-manager' ),
		);
		$_nMaxSamePass = $this->GetMaxSamePass();
		if ( $_nMaxSamePass ) {
			$rules[] = sprintf( esc_html__( 'not be one of the previous %d used passwords.', 'wp-password-policy-manager' ), $_nMaxSamePass );
		} else {
			$rules[] = esc_html__( 'not be the same as the previous one', 'wp-password-policy-manager' );
		}

		if ( ! ! ( $c = $this->GetPasswordLen() ) ) {
			$rules[] = sprintf( esc_html__( 'be at least %d characters long', 'wp-password-policy-manager' ), $c );
		}
		if ( $this->IsPolicyEnabled( self::POLICY_MIXCASE ) ) {
			$rules[] = sprintf( esc_html__( 'contain mixed case characters', 'wp-password-policy-manager' ) );
		}
		if ( $this->IsPolicyEnabled( self::POLICY_NUMBERS ) ) {
			$rules[] = sprintf( esc_html__( 'contain numeric digits', 'wp-password-policy-manager' ) );
		}
		if ( $this->IsPolicyEnabled( self::POLICY_SPECIAL ) ) {
			$rules[] = sprintf( esc_html__( 'contain special characters', 'wp-password-policy-manager' ) );
		}
	...
	function ModifyLoginForm() {
		if ( ! empty( $this->CurrentUserLogin ) ) {
			...
			$username = wp_unslash( isset( $_REQUEST['log'] ) && ! empty( $_REQUEST['log'] ) ? $_REQUEST['log'] : '' );
			if ( empty( $username ) ) {
				return;
			}
			if ( ! username_exists( $username ) ) {
		...
		wp_localize_script( 'front-js', 'wppm_ModifyForm', array(
			...
			'TextOldPass'       => esc_html__( 'Old Password', 'wp-password-policy-manager' ),
			'BtnChangeAndLogin' => esc_html__( 'Change & Log in', 'wp-password-policy-manager' ),
			'NewPasswordRules'  => $this->GetPasswordRules(),
			'NewPassRulesHead'  => esc_html__( 'New password must...', 'wp-password-policy-manager' ),
			'NewPassRulesFoot'  => esc_html__( 'WordPress Password Policies by', 'wp-password-policy-manager' ) . '<br/><a href="http://www.wpwhitesecurity.com/wordpress-security-plugins/wp-password-policy-manager/" target="_blank">' . esc_html__( 'WP Password Policy Manager', 'wp-password-policy-manager' ) . '</a>',
		) );

		?>
		<p>
			<label for="user_pass_new"><?php esc_html_e( 'New Password', 'wp-password-policy-manager' ); ?><br/>
				<input type="password" name="user_pass_new" id="user_pass_new" class="input"
				       placeholder="<?php esc_html_e( 'New Password', 'wp-password-policy-manager' ); ?>"
				       value="<?php echo ''; ?>" size="25"/></label>
		</p>
		<p>
			<label for="user_pass_vfy"><?php esc_html_e( 'Verify Password', 'wp-password-policy-manager' ); ?><br/>
				<input type="password" name="user_pass_vfy" id="user_pass_vfy" class="input"
				       placeholder="<?php esc_attr_e( 'Verify Password', 'wp-password-policy-manager' ); ?>"
				       value="<?php echo ''; ?>" size="25"/></label>
		</p>
		<?php
	}
	...
	public function ValidateLoginForm( $user, $password ) {
		if ( ! ( $user instanceof WP_User ) ) {
			return new WP_Error( 'expired_password', esc_html__( 'Invalid Request', 'wp-password-policy-manager' ) );
		}
		if ( $this->IsUserExemptFromPolicies( $user ) ) {
			return $user;
		}
		...
		if ( wp_check_password( $password, $user->data->user_pass, $user->ID ) && ( $wasReset || $this->IsUserPasswordOld( $user ) ) ) {
			...
			if ( isset( $_REQUEST['user_pass_new'] ) && isset( $_REQUEST['user_pass_vfy'] ) ) {
				if ( ! trim( $_REQUEST['user_pass_new'] ) || ! trim( $_REQUEST['user_pass_vfy'] ) ) {
					return new WP_Error( 'expired_password', sprintf( esc_html__( '%s ERROR %s: The new password cannot be empty.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				}
				if ( $_REQUEST['user_pass_new'] != $_REQUEST['user_pass_vfy'] ) {
					return new WP_Error( 'expired_password', sprintf( esc_html__( '%s ERROR %s: Both new passwords must match.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				}
				if ( wp_check_password( $_REQUEST['user_pass_new'], $user->data->user_pass, $user->ID ) ) {
					return new WP_Error( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password cannot be the same as the old one.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				}
				if ( $_REQUEST['user_pass_new'] == $user->user_login ) {
					return new WP_Error( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password cannot be the same as the username.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				}
				if ( $_REQUEST['user_pass_new'] == $user->user_email ) {
					return new WP_Error( 'expired_password', sprintf( esc_html__( '%sERROR%s: New password cannot be the same as the email.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				}
				if ( ( $c = $this->GetPasswordLen() ) != 0 ) {
					if ( strlen( $_REQUEST['user_pass_new'] ) < $c ) {
						return new WP_Error( 'expired_password', sprintf( esc_html__( '%sERROR%s: New password must contain at least %d characters.', 'wp-password-policy-manager' ), '<strong>', '</strong>', $c ) );
					}
				}
				if ( $this->IsPolicyEnabled( self::POLICY_MIXCASE ) ) {
					if ( strtolower( $_REQUEST['user_pass_new'] ) == $_REQUEST['user_pass_new'] ) {
						return new WP_Error( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password must contain both uppercase and lowercase characters.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
					}
				}
				if ( $this->IsPolicyEnabled( self::POLICY_NUMBERS ) ) {
					if ( ! preg_match( '/[0-9]/', $_REQUEST['user_pass_new'] ) ) {
						return new WP_Error( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password must contain numbers.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
					}
				}
				if ( $this->IsPolicyEnabled( self::POLICY_SPECIAL ) ) {
					if ( ! preg_match( '/[_\W]/', $_REQUEST['user_pass_new'] ) ) {
						return new WP_Error( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password must contain special characters.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
					}
				}
				...
				if ( $_nMaxSamePass ) {
					if ( $this->_pwdHasBeenUsed( $user->ID, $_REQUEST['user_pass_new'] ) ) {
						return new WP_Error( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password must not be one of the previous %d used passwords.', 'wp-password-policy-manager' ), '<strong>', '</strong>', $_nMaxSamePass ) );
					}
				...
				if ( $wasReset ) {
					$diff = esc_html__( '1 minute', 'wp-password-policy-manager' );
				} else {
					$diff = human_time_diff( strtotime( $this->GetPasswordTtl(), $this->GetPasswordLastModTime( $user->ID ) ), current_time( 'timestamp' ) );
				}

				return new WP_Error( 'expired_password', sprintf( esc_html__( '%s ERROR %s: The password you entered expired %s ago.', 'wp-password-policy-manager' ), '<strong>', '</strong>', $diff ) );
			}
		}
	...
	public function ModifyUserProfilePage( $user = null ) {
		...
		<table class="form-table">
			<?php if ( $this->IsPolicyEnabled( self::POLICY_OLDPASSWORD ) && ! $this->UserCanSkipOldPwdPolicy() ) { ?>
				<tr>
					<th><label
							for="wppmoldpass"><?php esc_html_e( 'Current Password', 'wp-password-policy-manager' ); ?></label>
					</th>
					<td>
						<input type="password" name="wppmoldpass" id="wppmoldpass" class="regular-text" size="16"
						       value=""
						       placeholder="<?php esc_html_e( 'Current Password', 'wp-password-policy-manager' ); ?>"
						       autocomplete="off"><br>
						<span
							class="description"><?php esc_html_e( 'Type your current password to be able to change your password.', 'wp-password-policy-manager' ); ?></span>
					</td>
				</tr>
			<?php } ?>
			<tr>
				<th><label><?php esc_html_e( 'New password must', 'wp-password-policy-manager' ); ?></label></th>
				<td>
					<div id="wppmUserProfilePwdRulesContainer">
						...
						<div style="width: 240px;">
							<p style="text-align: center;"><?php echo esc_html_e( 'WordPress Password Policies by', 'wp-password-policy-manager' ) . '<br/><a href="http://www.wpwhitesecurity.com/wordpress-security-plugins/wp-password-policy-manager/" target="_blank">' . esc_html__( 'WP Password Policy Manager', 'wp-password-policy-manager' ) . '</a>'
								?></p>
						</div>
					</div>
				</td>
		...
		if ( ! $this->IsUserExemptFromPolicies( $user ) ) {
			...
			if ( ( isset( $pass1 ) && isset( $pass2 ) ) && ( ! empty( $pass1 ) && ! empty( $pass2 ) ) ) {
				if ( empty( $pass1 ) || empty( $pass2 ) ) {
					$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: The new password cannot be empty.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );

					return $errors;
				}
				if ( $pass1 <> $pass2 ) {
					$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: Both new passwords must match.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				...
				if ( $validateOldPass && empty( $oldpass ) ) {
					$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: Please enter the current password in the Current Password field.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				...
				if ( wp_check_password( $pass1, $crtPwd, $user->ID ) ) {
					$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password cannot be the same as the old one.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );

					return $errors;
				}
				// new password cannot be the same as the username
				if ( $pass1 == $userInfo->user_login ) {
					$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password cannot be the same as the username.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );

					return $errors;
				}
				// new password cannot be the same as the email
				if ( $pass1 == $userInfo->user_email ) {
					$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password cannot be the same as the email.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				...
				if ( ( $c = $this->GetPasswordLen() ) != 0 ) {
					if ( strlen( $pass1 ) < $c ) {
						$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password must contain at least %d characters.', 'wp-password-policy-manager' ), '<strong>', '</strong>', $c ) );
				...
				if ( $this->IsPolicyEnabled( self::POLICY_MIXCASE ) ) {
					if ( strtolower( $pass1 ) == $pass1 ) {
						$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password must contain both uppercase and lowercase characters.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				...
				if ( $this->IsPolicyEnabled( self::POLICY_NUMBERS ) ) {
					if ( ! preg_match( '/[0-9]/', $pass1 ) ) {
						$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password must contain numbers.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				...
				if ( $this->IsPolicyEnabled( self::POLICY_SPECIAL ) ) {
					if ( ! preg_match( '/[_\W]/', $pass1 ) ) {
						$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password must contain special characters.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				...
				if ( $validateOldPass ) {
					if ( ! wp_check_password( $oldpass, $crtPwd, $user->ID ) ) {
						$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: Current password is incorrect.', 'wp-password-policy-manager' ), '<strong>', '</strong>' ) );
				...
				if ( $_nMaxSamePass ) {
					if ( $this->_pwdHasBeenUsed( $user->ID, $pass1 ) ) {
						$errors->add( 'expired_password', sprintf( esc_html__( '%s ERROR %s: New password must not be one of the previous %d used passwords.', 'wp-password-policy-manager' ), '<strong>', '</strong>', $_nMaxSamePass ) );
	...
	public function ValidatePasswordReset( WP_Error $errors, $user ) {
		...
		if ( 'POST' == $rm ) {
			if ( ! isset( $_POST['pass1'] ) || ! isset( $_POST['pass2'] ) ) {
				$errors->add( 'expired_password', esc_html__( 'The form is not valid. Please refresh the page and try again.', 'wp-password-policy-manager' ) );

				return $errors;
			}
			if ( empty( $_POST['pass1'] ) ) {
				$errors->add( 'expired_password', esc_html__( 'Please provide your new password.', 'wp-password-policy-manager' ) );

				return $errors;
			}
			if ( empty( $_POST['pass2'] ) ) {
				$errors->add( 'expired_password', esc_html__( 'Please confirm your new password.', 'wp-password-policy-manager' ) );
			...
			if ( $password != $p2 ) {
				$errors->add( 'expired_password', esc_html__( 'Passwords must match.', 'wp-password-policy-manager' ) );
			...
			if ( wp_check_password( $password, $user->data->user_pass, $user->ID ) ) {
				$errors->add( 'expired_password', esc_html__( 'The new password cannot be the same as the current one.', 'wp-password-policy-manager' ) );
	...
	public function SetPasswordTtl( $newTtl ) {
		if ( trim( $newTtl ) ) {
			...
			if ( $time === false || $time < $now ) {
				throw new Exception( esc_html__( 'Password policy expiration time is not valid.', 'wp-password-policy-manager' ) );
			}
		} else {
			$newTtl = '';
		}
	...
	protected function EchoIdent( $name ) {
		echo esc_attr( self::DEF_PFX . '_' . $name );
	}
	...
	protected function IsJustInstalled() {
		return ( $this->GetGlobalOption( self::OPT_NAME_TTL ) === false ) || ( $this->GetGlobalOption( self::OPT_NAME_LEN ) === false );
	}
	...
	protected function UpdateWpOptions() {
		if ( ! empty( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'nonce_form' ) ) {
			...
			throw new Exception( esc_html__( 'Security check failed', 'wp-password-policy-manager' ) );
		}
	}
	...
	private function _resetPasswordsHelper( $blogId = 0, $exceptSuperAdmin = true, $usingCron = false ) {
		...
		foreach ( $users as $user ) {
			...
			$message .= '<p>' . sprintf( esc_html__( 'Your password for %s has been reset.', 'wp-password-policy-manager' ), '<strong>' . $blogname .'</strong>' ) . "</p>\r\n\r\n";
			$message .= '<p>' . sprintf( esc_html__( 'New Password: %s', 'wp-password-policy-manager' ), '<strong>' . $new_password . '</strong>' ) . "</p>\r\n\r\n";
			$message .= '<p>' . sprintf( esc_html__( 'Please log in and change your password:', 'wp-password-policy-manager' ) ) . "\r\n";
	...
	public function get_blogs_ajax() {
		...
		if ( ! isset( $_POST ) || empty( $_POST ) ) {
			wp_send_json_error( esc_html__( 'Invalid request', 'wp-password-policy-manager' ) );
		}
		...
		$query = $wpdb->prepare( "SELECT * FROM  {$wpdb->blogs} WHERE domain like %s AND spam = 0 AND deleted = 0 ORDER BY blog_id", '%' . $q . '%' );
	...
	protected function SendNotificationEmail( $emailAddress, $message ) {
		...
		$subject = esc_html__( 'Password has been reset', 'wp-password-policy-manager' );
	...
	public function ManageWpOptions() {
		...
		if ( ! $this->IsManagingAdmin() || $this->IsAdminAccessDisabled() ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'wp-password-policy-manager' ) );
		}
		...
		if ( isset( $_POST ) && count( $_POST ) ) {
			...
			if ( $this->IsMultisite() && is_super_admin() ) {
				...
				if ( isset( $_POST['wppm-reset-sites'] ) ) {
					if ( intval( $_POST['wppm-reset-sites'] ) < 1 ) {
						$__allBlogs = true;
					} else {
						$__blogId = absint( $_POST['wppm-reset-sites'] );
					}
				}
			...
			try {
				switch ( true ) {
					case isset( $_POST[ self::DEF_PFX . '_snt' ] ):
						...
						?>
						<div class="updated"><p>
							<strong><?php esc_html_e( 'Settings saved.', 'wp-password-policy-manager' ); ?></strong></p>
						</div><?php
						break;
						...
						if ( ! is_null( $bid ) ) {
							...
							if ( $this->IsPostIdent( 'wpcron' ) ) {
								...
								<div class="updated"><p><strong>
											<?php esc_html_e( 'Request registered. Passwords will be reset using WP Cron in 10 minutes.', 'wp-password-policy-manager' ); ?>
										</strong></p></div>
								<?php
							} else {
								$this->DeleteGlobalOption( self::CRON_RESET_PWD_OPT_NAME );
								...
								?>
								<div class="updated"><p>
									<strong><?php esc_html_e( 'All passwords have been reset.', 'wp-password-policy-manager' ); ?></strong>
								</p></div><?php
							}
						}
						break;
					default:
						throw new Exception( esc_html__( 'Unexpected form submission content.', 'wp-password-policy-manager' ) );
				}
			} catch ( Exception $ex ) {
				?>
				<div class="error"><p>
					<strong><?php echo esc_html__( 'Error', 'wp-password-policy-manager' ) . ': ' . esc_html( $ex->getMessage() ); ?></strong>
				</p></div><?php
			}
		}
		...
		?>
		<div class="wrap">
		<h2><?php esc_html_e( 'WordPress Password Policy Manager Settings', 'wp-password-policy-manager' ); ?></h2>
		<form method="post" id="wppm_settings">
			...
			<div id="wppm-adverts">
				<a href="http://www.wpwhitesecurity.com/plugins-premium-extensions/email-notifications-wordpress/?utm_source=wppmplugin&utm_medium=settingspage&utm_campaign=notifications"
				   target="_blank">
					<img src="<?php echo esc_url( $this->GetBaseUrl() ); ?>/img/notifications_250x150.gif" width="250" height="150"
					     alt="">
				</a>
				<a href="http://www.wpwhitesecurity.com/plugins-premium-extensions/search-filtering-extension/?utm_source=wppmplugin&utm_medium=settingspage&utm_campaign=search"
				   target="_blank">
					<img src="<?php echo esc_url( $this->GetBaseUrl() ); ?>/img/search_250x150.gif" width="250" height="150"
					     alt="">
				</a>
				<a href="http://www.wpwhitesecurity.com/plugins-premium-extensions/wordpress-reports-extension/?utm_source=wppmplugin&utm_medium=settingspage&utm_campaign=reports"
				   target="_blank">
					<img src="<?php echo esc_url( $this->GetBaseUrl() ); ?>/img/reporting_250x150.gif" width="250" height="150"
					     alt="">
				</a>
			</div>
			<table class="form-table">
				...
				<tr valign="top">
					<th scope="row"><label
							for="<?php $this->EchoIdent( 'ttl' ); ?>"><?php esc_html_e( 'Password Expiration Policy', 'wp-password-policy-manager' ); ?></label>
					</th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Examples:', 'wp-password-policy-manager' ); ?>
							<code><?php esc_html_e( '5 days', 'wp-password-policy-manager' ); ?></code>
							<code><?php esc_html_e( '20 days 6 hours', 'wp-password-policy-manager' ); ?></code>
							<code><?php esc_html_e( '3 weeks', 'wp-password-policy-manager' ); ?></code></p>
						<?php esc_html_e( 'Leave blank to disable Password Expiry policy.', 'wp-password-policy-manager' ); ?>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label
							for="<?php $this->EchoIdent( 'len' ); ?>"><?php esc_html_e( 'Password Length Policy', 'wp-password-policy-manager' ); ?></label>
					</th>
					<td>
						<select id="<?php $this->EchoIdent( 'len' ); ?>"
						        name="<?php $this->EchoIdent( 'len' ); ?>"><?php
							$curr = $this->GetPasswordLen();
							foreach ( array_merge( array( 0 ), range( 4, 16 ) ) as $value ) { ?>
							<option value="<?php echo esc_attr( $value ); ?>" <?php  selected( $value,$curr ); ?>>
								<?php echo esc_html( $value == 0 ? '' : $value ); ?>
								</option><?php
							}
							?></select> <?php esc_html_e( 'characters', 'wp-password-policy-manager' ); ?><br/>
						<?php esc_html_e( 'Leave blank to disable Password Length policy.', 'wp-password-policy-manager' ); ?>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label
							for="<?php $this->EchoIdent( 'cpt' ); ?>"><?php esc_html_e( 'Mixed Case Policy', 'wp-password-policy-manager' ); ?></label>
					</th>
					<td>
						<fieldset>
							<legend class="screen-reader-text">
								<span><?php esc_html_e( 'Mixed Case Policy', 'wp-password-policy-manager' ); ?></span></legend>
							<label for="<?php $this->EchoIdent( 'cpt' ); ?>">
								<input name="<?php $this->EchoIdent( 'cpt' ); ?>" type="checkbox"
								       id="<?php $this->EchoIdent( 'cpt' ); ?>"
								       value="1"<?php if ( $this->IsPolicyEnabled( self::POLICY_MIXCASE ) ) {
									echo ' checked="checked"';
								} ?>/>
								<?php esc_html_e( 'Password must contain a mix of uppercase and lowercase characters.', 'wp-password-policy-manager' ); ?>
							</label>
						</fieldset>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label
							for="<?php $this->EchoIdent( 'num' ); ?>"><?php esc_html_e( 'Numeric Digits Policy', 'wp-password-policy-manager' ); ?></label>
					</th>
					<td>
						<fieldset>
							<legend class="screen-reader-text">
								<span><?php esc_html_e( 'Numeric Digits Policy', 'wp-password-policy-manager' ); ?></span>
							</legend>
							<label for="<?php $this->EchoIdent( 'num' ); ?>">
								<input name="<?php $this->EchoIdent( 'num' ); ?>" type="checkbox"
								       id="<?php $this->EchoIdent( 'num' ); ?>"
								       value="1"<?php if ( $this->IsPolicyEnabled( self::POLICY_NUMBERS ) ) {
									echo ' checked="checked"';
								} ?>/>
								<?php printf( esc_html__( 'Password must contain numeric digits (%s).', 'wp-password-policy-manager' ), '<code>0-9</code>' ); ?>
							</label>
						</fieldset>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label
							for="<?php $this->EchoIdent( 'spc' ); ?>"><?php esc_html_e( 'Special Characters Policy', 'wp-password-policy-manager' ); ?></label>
					</th>
					<td>
						<fieldset>
							<legend class="screen-reader-text">
								<span><?php esc_html_e( 'Special Characters Policy', 'wp-password-policy-manager' ); ?></span>
							</legend>
							<label for="<?php $this->EchoIdent( 'spc' ); ?>">
								<input name="<?php $this->EchoIdent( 'spc' ); ?>" type="checkbox"
								       id="<?php $this->EchoIdent( 'spc' ); ?>"
								       value="1"<?php if ( $this->IsPolicyEnabled( self::POLICY_SPECIAL ) ) {
									echo ' checked="checked"';
								} ?>/>
								<?php printf( esc_html__( 'Password must contain special characters (eg: %s).', 'wp-password-policy-manager' ), '<code>.,!#$_+</code>' ); ?>
							</label>
						</fieldset>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label
							for="<?php $this->EchoIdent( 'opw' ); ?>"><?php esc_html_e( 'Current Password Policy', 'wp-password-policy-manager' ); ?></label>
					</th>
					<td>
						<fieldset>
							<legend class="screen-reader-text">
								<span><?php esc_html_e( 'Current Password Policy', 'wp-password-policy-manager' ); ?></span>
							</legend>
							<label for="<?php $this->EchoIdent( 'opw' ); ?>">
								<input name="<?php $this->EchoIdent( 'opw' ); ?>" type="checkbox"
								       id="<?php $this->EchoIdent( 'opw' ); ?>"
								       value="1"<?php if ( $this->IsPolicyEnabled( self::POLICY_OLDPASSWORD ) ) {
									echo ' checked="checked"';
								} ?>/>
								<?php esc_html_e( 'When changing password on the profile page, the user must supply the current password.', 'wp-password-policy-manager' ); ?>
							</label>
						</fieldset>
					</td>
				</tr>
				<tr valign="top">
					<th><label
							for="<?php $this->EchoIdent( 'msp' ); ?>"><?php esc_html_e( 'Password History Policy', 'wp-password-policy-manager' ); ?></label>
					</th>
					<td>
						<fieldset>
							<?php esc_html_e( 'Remember', 'wp-password-policy-manager' ); ?>
							<select id="<?php $this->EchoIdent( 'msp' ); ?>"
								...
								foreach ( array_merge( array( 0 ), range( 2, 10 ) ) as $value ) {
									...
									?>
								<option value="<?php echo esc_attr( $value ); ?>"<?php selected( $value, $crt ); ?>>
									<?php echo esc_html( $value == 0 ? '' : $value ); ?>
									</option><?php
								}
								?></select> <?php esc_html_e( 'old passwords', 'wp-password-policy-manager' ); ?><br/>
							<?php esc_html_e( 'Leave blank to disable password history policy.', 'wp-password-policy-manager' ); ?>
						</fieldset>
					</td>
				</tr>
				...
				<?php if ( $this->IsMultisite() && is_super_admin() ) : ?>
					<tr valign="top">
						<th><label
								for="<?php $this->EchoIdent( 'daa' ); ?>"><?php esc_html_e( 'Disable Admins Access', 'wp-password-policy-manager' ); ?></label>
						</th>
						<td>
							<fieldset>
								<legend class="screen-reader-text">
									<span><?php esc_html_e( 'Disable Admins Access', 'wp-password-policy-manager' ); ?></span>
								</legend>
								<label for="<?php $this->EchoIdent( 'daa' ); ?>">
									...
									/>
									<?php esc_html_e( "Disallow site administrators from modifying the plugin's settings.", 'wp-password-policy-manager' ); ?>
								</label>
							</fieldset>
						</td>
					</tr>
				...
				<tr>
					<th><label
							for="ExemptTokenQueryBox"><?php esc_html_e( 'Users and Roles Exempt From Policies', 'wp-password-policy-manager' ); ?></label>
					</th>
					<td>
						<fieldset>
							...
							<p class="description">
								<?php
								esc_html_e( 'Users and Roles in this list are free of all Password Policies.', 'wp-password-policy-manager' );
								?>
							</p>
							<div id="ExemptTokenList"><?php
								foreach ( $this->GetExemptTokens() as $item ) {
									...
									<input type="hidden" name="ExemptTokens[]"
				...
				<tr valign="top">
					<th scope="row"><label
							for="rst-submit-button"><?php esc_html_e( "Reset All Users' Passwords", 'wp-password-policy-manager' ); ?></label>
					</th>
					<td>
						<?php if ( $this->IsMultisite() && is_super_admin() ) { ?>
							<div id="sa_options_container" style="margin: 10px 0 10px 0;">
								<div>
									<label for="wppm-all-sites">
										<?php esc_html_e( 'All sites on network:', 'wp-password-policy-manager' ); ?>
										<input id="wppm-all-sites" name="wppm-reset-sites" type="radio" value="-1"
										       style="margin-left: 10px;"/>
									</label>
									<br/>
									<label for="wppm-specific-site">
										<?php esc_html_e( 'Specific site on Network:', 'wp-password-policy-manager' ); ?>
										<input id="wppm-specific-site" name="wppm-reset-sites" type="radio"
										       style="margin-left: 10px;"
										       placeholder="<?php esc_attr_e( 'Search for a site', 'wp-password-policy-manager' ); ?>"/>
									</label>
								</div>
							</div>
						...
						<script type="text/javascript">
							jQuery( function( $ ) {
								$( '#rst-submit-button' ).on( 'click', function() {
									if ( confirm( "<?php echo esc_js( esc_attr__( 'Are you sure you want to reset all passwords?', 'wp-password-policy-manager' ) );?>" ) ) {
						...
						<input id="rst-submit-button" type="submit"
						       ...
						       value="<?php esc_attr_e( esc_html__( 'Reset All Passwords', 'wp-password-policy-manager' ) ); ?>"/>
					</td>
				</tr>
				<?php if ( ! defined( 'DISABLE_WP_CRON' ) || ! DISABLE_WP_CRON ) { ?>
					<tr valign="top">
						<th scope="row"><label
								for="<?php $this->EchoIdent( 'wpcron' ); ?>"><?php esc_html_e( 'Use WP Cron', 'wp-password-policy-manager' ); ?></label>
						</th>
						<td>
							<?php
							...
							<p class="description"><?php esc_html_e( 'Only check this option if your site has many users.', 'wp-password-policy-manager' ); ?></p>
						</td>
					</tr>
				<?php } ?>
				</tbody>
			</table>
			<!-- Policy Flags: <?php echo esc_html( $this->_policy_flag_cache ); ?> -->
			<p class="submit">
				<input type="submit" name="<?php $this->EchoIdent( 'snt' ); ?>" class="button-primary"
				       value="<?php echo esc_attr( esc_html__( 'Save Changes', 'wp-password-policy-manager' ) ); ?>"/>
			</p>
		</form>
		</div><?php
	}
	...
	public function GetPasswordLastModTime( $user_ID ) {
		...
		if ( ! $time ) {
			$time = strtotime( get_userdata( $user_ID )->user_registered );
		}
	...
	public function plugin_action_links( $old_links ) {
		$new_links = array(
			'<a href="' . admin_url( 'options-general.php?page=password_policy_settings' ) . '">' . esc_html__( 'Configure Password Policies', 'wp-password-policy-manager' ) . '</a>',
		);
	...
	public function admin_menu() {
		add_options_page( esc_html__( 'Password Policies', 'wp-password-policy-manager' ), esc_html__( 'Password Policies', 'wp-password-policy-manager' ), 'manage_options', self::PLG_CONFIG_MENU_NAME, array(
			$this,
			'ManageWpOptions',
		) );
	}
	...
	public function network_admin_menu() {
		add_options_page( esc_html__( 'Password Policies', 'wp-password-policy-manager' ), esc_html__( 'Password Policies', 'wp-password-policy-manager' ), 'manage_network', self::PLG_CONFIG_MENU_NAME, array(
			$this,
			'ManageWpOptions',
		) );
		add_submenu_page( 'settings.php', esc_html__( 'Password Policies', 'wp-password-policy-manager' ), esc_html__( 'Password Policies', 'wp-password-policy-manager' ), 'manage_network_options', self::PLG_CONFIG_MENU_NAME, array(
			$this,
			'ManageWpOptions',
		) );
	}
	...
	public function admin_print_footer_scripts() {
		...
		if ( $this->IsJustInstalled() && $this->IsManagingAdmin() && ! $isOnPluginPage ) {
			$tle = esc_html__( 'Configure Password Policies', 'wp-password-policy-manager' );
			$txt = esc_html__( 'You have just installed WP Password Policy manager. All password policies are disabled by default. Click the button below to configure the WordPress password policies.', 'wp-password-policy-manager' );
			$btn = esc_html__( 'Configure Policies', 'wp-password-policy-manager' );
	...
	public function wp_ajax_check_security_token() {
		if ( ! $this->IsManagingAdmin() ) {
			die( esc_html__( 'Access Denied.', 'wp-password-policy-manager' ) );
		}
		if ( ! isset( $_REQUEST['token'] ) ) {
			die( esc_html__( 'Token parameter expected.', 'wp-password-policy-manager' ) );
		}
	...
	public function _cronAddCustomInterval( $schedules ) {
		...
		$schedules['ten_minutes'] = array(
			...
			'display'  => esc_html__( 'Once Every 10 Minutes', 'wp-password-policy-manager' ),
		);
